...
# Flags in IP Header
# 0x02 ^= DF-Bit
NMAP_PROBE_IP_ATTR = {"T2": {"FLGS": 0x02}, "T4": {"FLGS": 0x02}, "T6": {"FLGS": 0x02}}
    ...
    def set_IN_SEQ_NR(self, seqn):
        ...
        if seqn == "O":
            ...
            if 0 < self.os_pattern.GCD < 9:
                ...
                while (
                    self.os_pattern.SEQ_MAX < (self.os_pattern.SEQNr_mean + temp)
                ) or (self.os_pattern.SEQ_MIN > (self.os_pattern.SEQNr_mean + temp)):
                    temp = random.randint(0, self.os_pattern.SEQ_std_dev)

                ISN_delta += temp

                self.os_pattern.TCP_SEQ_NR_tmp = (
                    self.os_pattern.TCP_SEQ_NR_tmp + ISN_delta
                ) % 2 ** 32
                self.tcp.seq = self.os_pattern.TCP_SEQ_NR_tmp

            else:
                self.os_pattern.TCP_SEQ_NR_tmp = (
                    self.os_pattern.TCP_SEQ_NR_tmp + self.os_pattern.SEQNr_mean
                ) % 2 ** 32
                self.tcp.seq = self.os_pattern.TCP_SEQ_NR_tmp
...
def send_TCP_reply(pkt, os_pattern, TCP_OPTIONS, flags, ipid=0, seqn="O", ack="S+"):
    ...
    if TCP_OPTIONS["O"]:
        ...
        tcp_rpl.set_TCP_Options(
            TCP_OPTIONS["O"]
            + [("Timestamp", (int(str(os_pattern.TCP_Timestamp_tmp), 10), tsver))]
        )
    ...
    if check_TCP_Nmap_match(
        ...
        if os_pattern.PROBES_2_SEND["P1"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["P1"],
                os_pattern.TCP_FLAGS["SEQ"],
                os_pattern.IP_ID_TI_CNT,
            )
            # print "TCP Probe #1"
    ...
    # ECN
        ...
        if os_pattern.PROBES_2_SEND["ECN"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["ECN"],
                os_pattern.TCP_FLAGS["ECN"],
                os_pattern.IP_ID_TI_CNT,
                ECN_URGT_PTR,
            )
            # print "TCP Probe #ECN"
    ...
    # T2-T7
        ...
        if os_pattern.PROBES_2_SEND["T2"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T2"],
                os_pattern.TCP_FLAGS["T2"],
                0,
                os_pattern.TCP_SEQ_NR["T2"],
                os_pattern.TCP_ACK_NR["T2"],
            )
            # print "TCP Probe #T2"
        ...
        if os_pattern.PROBES_2_SEND["T3"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T3"],
                os_pattern.TCP_FLAGS["T3"],
                0,
                os_pattern.TCP_SEQ_NR["T3"],
                os_pattern.TCP_ACK_NR["T3"],
            )
            # print "TCP Probe #T3"
        ...
        if os_pattern.PROBES_2_SEND["T4"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T4"],
                os_pattern.TCP_FLAGS["T4"],
                0,
                os_pattern.TCP_SEQ_NR["T4"],
                os_pattern.TCP_ACK_NR["T4"],
            )
            # print "TCP Probe #T4"
        ...
        if os_pattern.PROBES_2_SEND["T5"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T5"],
                os_pattern.TCP_FLAGS["T5"],
                os_pattern.IP_ID_CI_CNT,
                os_pattern.TCP_SEQ_NR["T5"],
                os_pattern.TCP_ACK_NR["T5"],
            )
            # print "TCP Probe #T5"
        ...
        if os_pattern.PROBES_2_SEND["T6"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T6"],
                os_pattern.TCP_FLAGS["T6"],
                os_pattern.IP_ID_CI_CNT,
                os_pattern.TCP_SEQ_NR["T6"],
                os_pattern.TCP_ACK_NR["T6"],
            )
            # print "TCP Probe #T6"
        ...
        if os_pattern.PROBES_2_SEND["T7"]:
            ...
            send_TCP_reply(
                pkt,
                os_pattern,
                os_pattern.TCP_OPTIONS["T7"],
                os_pattern.TCP_FLAGS["T7"],
                os_pattern.IP_ID_CI_CNT,
                os_pattern.TCP_SEQ_NR["T7"],
                os_pattern.TCP_ACK_NR["T7"],
            )
            # print "TCP Probe #T7"
